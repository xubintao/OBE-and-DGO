...
def otsu_threshold(gray_roi):

    # 计算灰度直方图
    hist, _ = np.histogram(gray_roi.flatten(), bins=256, range=(0,256))
    
    # 归一化直方图
    hist_norm = hist / hist.sum()
    
    # 计算类内方差
    weight = np.arange(256)
    mean = np.average(weight, weights=hist_norm)
    inter_class_variance = np.sum(((weight - mean) ** 2) * hist_norm)
    ...
    for t in range(256):
        ...
        if w0 == 0 or w1 == 0:
            continue
        mean0 = np.average(weight[:t], weights=hist_norm[:t])
        mean1 = np.average(weight[t:], weights=hist_norm[t:])
...
def enhance_brightness(image, bbox, multiplier=1.3):
    ...
    # 计算阈值
    threshold = otsu_threshold(gray_roi)
    # print(threshold)
