...
def draw_coordinate(draw,
    ...
    """
    将目标真实坐标信息绘制到图片上
    #基于“K387+193_K387+223”这种格式才可以使用该函数
    """
    #首先通过传入的参数对字体进行初始化，如果指定的字体文件不存在，则使用默认字体
    try:
    ...
    #计算真实坐标
    front_coordinate=img_name.split("_")[0]
    left, top, right, bottom = box
    middle=(left+right)/2
    behind_coordinate=round(0.02008*middle+float(front_coordinate.split("+")[1]),2)
    coordinate=front_coordinate.split("+")[0]+"+"+str(behind_coordinate)

    #计算字符串高度
    display_str_heights = [font.getsize(ds)[1] for ds in coordinate]
...
def draw_coordinate(draw,
    ...
    left, top, right, bottom = box
    middle = (left + right) / 2
    # real_middle = middle - 286 / 4    #原图的白边宽度为286pixel，预测的图要除以4，所以他也要
    # behind_coordinate = round(0.02008 * real_middle + float(front_coordinate.split("+")[1]), 2)
    real_middle = middle #第三批图像预处理后没有白边
    behind_coordinate = round(0.00368 * real_middle + float(front_coordinate.split("+")[1]), 2)
    coordinate = front_coordinate.split("+")[0] + "+\n" + str(behind_coordinate)

    # 计算字符串高度
    display_str_heights = [font.getsize(ds)[1] for ds in coordinate.split("\n")]
    # Each display_str has a top and bottom margin of 0.05x.
    display_str_height = (1 + 2 * 0.05) * max(display_str_heights)

    # 计算字符串坐标
    text_top = bottom
    text_bottom = bottom + display_str_height*2

    for ds in coordinate.split("\n"):
        text_width, text_height = font.getsize(ds)
        margin = np.ceil(0.05 * text_width)
        draw.rectangle([(left, text_top),
                        (left + text_width + 2 * margin, text_bottom)], fill=color)
...
def draw_objs(image: Image,
    ...
    if draw_boxes_on_image:
        ...
        for box, cls, score, color in zip(boxes, classes, scores, colors):
            ...
            # 绘制目标坐标
            # draw_coordinate(draw,img_name,box.tolist(),color, font, font_size)
