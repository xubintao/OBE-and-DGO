...
def create_model(num_classes, load_pretrain_weights=True):
    ...
    backbone = resnet50_fpn_backbone(pretrain_path="pre_weights/resnet50.pth", trainable_layers=3)

    model = MaskRCNN(backbone, num_classes=num_classes)

    if load_pretrain_weights:
        # coco weights url: "https://download.pytorch.org/models/maskrcnn_resnet50_fpn_coco-bf2d0c1e.pth"
        weights_dict = torch.load("pre_weights/maskrcnn_resnet50_fpn_coco.pth", map_location="cpu")
...
def main(args):
    ...
    # device = torch.device(args.device if None else "cpu")
...
if __name__ == "__main__":
    ...
    # 训练数据集的根目录
    parser.add_argument('--data-path', default='E:/deep-learning-for-image-processing-pytorch_object_detection/VOCtrainval_11-May-2012/VOCdevkit', help='dataset')
    # 检测目标类别数(不包含背景)
